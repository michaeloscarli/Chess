...
public class Board {
	...
	/**
	 * Undoes the last movement done on the board
	 */
	public void undo()
	{
		if(lastPieceTaken!=null)
		{
			addPiece(lastPieceTaken, lastPieceMoved.getXLocation(), lastPieceMoved.getYLocation());
		}
		removePiece(lastPieceMoved.getXLocation(), lastPieceMoved.getYLocation());
		
		addPiece(lastPieceMoved, (int)lastPoint.getX(), (int)lastPoint.getY());
	...
	public boolean move(Piece piece, int newX, int newY)
	{
		...
		lastPoint = new Point(oldX, oldY);
		if (newX == oldX && newY == oldY)
			return true;
		if (board[newX][newY].getPiece()!=null)
			lastPieceTaken = board[newX][newY].getPiece();
